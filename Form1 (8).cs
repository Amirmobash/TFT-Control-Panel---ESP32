        ...
        // ---- Time preset buttons (increase number of hour buttons) ----
        private readonly List<Button> _dynamicTimeButtons = new();
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            // Apply orange/white theme
            ApplyOrangeWhiteTheme();

            // Increase time preset buttons (more hour options)
            SetupTimePresetButtons();
            ...
            this.Resize += (_, __) =>
            {
                ...
                // panel6 might need scroll update when window changes
                try { panel6?.Invalidate(); } catch { }
            };
...
//System.Threading.Timer timer = new System.Threading.Timer(LiveScreenshot, null, 0, 2000); // (2 start)
        ...
        public void Zero()
        {
            ...
            if (itemRows.Count > 0)
            {
                ...
                // IMPORTANT:
                // Zero() is called when a barcode is finalized / a new box starts.
                // The labels must be reset (per-box counters), BUT chart5 holds the
                // cumulative totals for the whole session/shift, so it must NOT be reset.
            }
            else
            {
                // legacy 4 items
        ...
        // -------------------- Theme (Orange / White) --------------------
        private void ApplyOrangeWhiteTheme()
        {
            try
            {
                // base
                ...
                // items + results backgrounds
                if (itemsHost != null) itemsHost.BackColor = Color.White;
                if (resultsHost != null) resultsHost.BackColor = Color.White;
                if (resultsRow != null) resultsRow.BackColor = Color.White;
                ...
                try
                {
                    ...
                    dataGridView1.ColumnHeadersDefaultCellStyle.BackColor = ThemeLightOrange;
                    dataGridView1.ColumnHeadersDefaultCellStyle.ForeColor = ThemeText;
                    dataGridView1.DefaultCellStyle.BackColor = Color.White;
                    dataGridView1.DefaultCellStyle.ForeColor = ThemeText;
                }
                catch { }
                ...
                {
                    r.Panel.BackColor = (r.Index % 2 == 0) ? ThemeLightOrange : Color.White;

                    if (r.NameBox != null)
                    {
                        r.NameBox.BackColor = Color.White;
                        r.NameBox.ForeColor = ThemeOrange;
                        r.NameBox.BorderStyle = BorderStyle.FixedSingle;
                    }

                    foreach (var lbl in r.ShiftLabels)
                    {
                    ...
                    foreach (Control c in r.Panel.Controls)
                    {
                        if (c is Button b)
                        {
                            ...
                            b.FlatAppearance.BorderColor = ThemeOrange;
                            b.FlatAppearance.BorderSize = 1;

                            // + buttons orange, - button white
                            if (b.Text.StartsWith("+"))
                            {
                                b.BackColor = ThemeOrange;
                                b.ForeColor = Color.White;
                            }
                            else
                            {
                                b.BackColor = Color.White;
                                // keep red for -1 readability
                ...
                // style other buttons (optional - keep icons as-is)
                foreach (Control c in this.Controls)
                {
                    StyleButtonsRecursive(c);
                }

                // style result labels
                try
                {
                    totalNumber.ForeColor = ThemeText;
                    totalBox.ForeColor = ThemeText;
                    totalHours.ForeColor = ThemeText;
                    STHLabel.ForeColor = ThemeText;
                }
                catch { }
            }
            catch
        ...
        private void StyleButtonsRecursive(Control root)
        {
            foreach (Control c in root.Controls)
            {
                if (c is Button b)
                {
                    // do not override dynamic row buttons (already styled)
                    if (b.Tag is Tuple<int, int>)
                    {
                        // skip
                    }
                    else
                    {
                        // keep image buttons
                        if (b.Image == null && (b.Text?.Length ?? 0) > 0)
                        {
                            b.UseVisualStyleBackColor = false;
                            b.FlatStyle = FlatStyle.Flat;
                            b.FlatAppearance.BorderColor = ThemeOrange;
                ...
                if (c is Panel p)
                {
                    // keep dynamic row colors and results host
                    bool isDynamic = itemRows.Any(r => ReferenceEquals(r.Panel, p));
                    if (!isDynamic && !ReferenceEquals(p, resultsHost))
                    {
        ...
        // -------------------- Time Presets (More hour buttons) --------------------
        // Adds more start/end time buttons into panel6 so user can choose hours faster.
        // We generate buttons at runtime to avoid heavy Designer edits.
        private void SetupTimePresetButtons()
        {
            try
            {
                if (panel6 == null)
                ...
                // Hide any old designer time buttons to prevent overlap.
                foreach (var b in panel6.Controls.OfType<Button>())
                    b.Visible = false;

                // Remove previously generated buttons (if any)
                foreach (var b in _dynamicTimeButtons)
                {
                    try { panel6.Controls.Remove(b); b.Dispose(); } catch { }
                }
                _dynamicTimeButtons.Clear();

                panel6.AutoScroll = true;

                // Times list (you can edit these)
                // Start row: from 04:00 to 15:00 every 30 minutes
                var startTimes = BuildHalfHourTimes(4, 0, 15, 0);
                // End row: from 13:00 to 23:30 every 30 minutes
                var endTimes = BuildHalfHourTimes(13, 0, 23, 30);

                const int topY = 5;
                const int bottomY = 46;
                const int gap = 6;
                const int btnW = 63;
                const int btnH = 34;
                int x = 6;

                // Start time buttons (top row)
                foreach (var t in startTimes)
                {
                    var btn = CreateTimeButton(t, x, topY, btnW, btnH, startTimeButtons);
                    panel6.Controls.Add(btn);
                    _dynamicTimeButtons.Add(btn);
                    x += btnW + gap;
                }

                // End time buttons (bottom row)
                x = 6;
                foreach (var t in endTimes)
                {
                    var btn = CreateTimeButton(t, x, bottomY, btnW, btnH, endTimeButtons);
                    panel6.Controls.Add(btn);
                    _dynamicTimeButtons.Add(btn);
                    x += btnW + gap;
                }

                // Keep panel size; scrolling will appear only if needed.
                panel6.Invalidate();
            }
            catch
            {
        ...
        private static List<string> BuildHalfHourTimes(int startHour, int startMinute, int endHour, int endMinute)
        {
            var list = new List<string>();
            var t = new TimeSpan(startHour, startMinute, 0);
            var end = new TimeSpan(endHour, endMinute, 0);

            // include end
            while (t <= end)
            {
                // show like designer: "4" for full hours, "4:30" for half
                if (t.Minutes == 0)
                    list.Add(t.Hours.ToString());
                else
                    list.Add($"{t.Hours}:{t.Minutes:00}");
                t = t.Add(TimeSpan.FromMinutes(30));
            }
            return list;
        }

        private Button CreateTimeButton(string text, int x, int y, int w, int h, EventHandler click)
        {
            var b = new Button
            {
            ...
            // apply theme to generated time buttons
            try
            {
                b.UseVisualStyleBackColor = false;
                b.FlatStyle = FlatStyle.Flat;
...
// -------------------- Results layout (4 numbers at bottom in one row) --------------------
        ...
        {
            // orange/white theme: alternate light orange and white rows
            var panel = new Panel
            {
                Width = panel1.Width,
                Height = panel1.Height,
                BackColor = (index % 2 == 0) ? ThemeLightOrange : Color.White,
