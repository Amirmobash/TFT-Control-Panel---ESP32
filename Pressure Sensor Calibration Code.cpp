...
struct Settings {
  ...
  float pressureThreshold;    // Nur für Anzeige (veraltet)
...
void drawPressureGauge() {
  ...
  tft.fillRoundRect(18, CONTENT_Y + 164, SCREEN_W - 36, 44, 12, statusColor);
  tft.drawRoundRect(18, CONTENT_Y + 164, SCREEN_W - 36, 44, 12, ORANGE_LIGHT);
  
  setTextStyle(2, BLACK);
  int textWidth = strlen(statusText) * 12;
  tft.setCursor((SCREEN_W - textWidth) / 2, CONTENT_Y + 178);
        ...
        <div class="grid">
            ...
            <div class="card">
                ...
                <div class="setting-group">
                    <label class="setting-label">Druckskala (bar/V): <input type="number" id="scale" min="0.1" max="10" step="0.001" value="1.0"></label>
                </div>
                ...
                <div class="setting-group">
                    <label class="setting-label">OK-Bereich Minimum: <input type="number" id="okMin" min="0.0" max="5.0" step="0.01" value="0.40"> bar</label>
                    <label class="setting-label">OK-Bereich Maximum: <input type="number" id="okMax" min="0.0" max="5.0" step="0.01" value="0.50"> bar</label>
                </div>
                ...
                <div class="calibration-section">
                    ...
                    <p style="color: var(--text-medium); font-size: 0.9em; margin-top: 15px; line-height: 1.5;">
                        Anleitung:<br>
                        1. <strong>Null kalibrieren:</strong> Sensor in offener Luft (0 bar)<br>
                        2. Druck sollte nun zwischen 0.40 und 0.50 bar liegen für "OK"
                    </p>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        async function updateStatus() {
            try {
                ...
                pressureStatusEl.textContent = data.pressureStatusText;
                pressureStatusEl.className = 'status-value ' + 
                    (data.pressureStatus === 0 ? 'status-ok' : 
                     data.pressureStatus === 1 ? 'status-low' : 'status-high');
                ...
                document.getElementById('okMin').value = data.okMin;
                document.getElementById('okMax').value = data.okMax;
        ...
        async function saveSettings() {
            const settings = {
                ...
                okMin: parseFloat(document.getElementById('okMin').value),
                okMax: parseFloat(document.getElementById('okMax').value),
        ...
        async function toggleRelay() {
            try {
                ...
                showNotification('Fehler beim Umschalten des Relais!', 'error');
                console.error('Error:', error);
            }
        }
...
void handleStatus() {
  ...
  json += "\"pressureStatusText\":\"" + String(getPressureStatusText()) + "\",";
