...
struct Settings {
  ...
  float pressureScale;        // bar/V (wird automatisch berechnet bei Span-Kalibrierung)
  int joyOnThreshold;
  int joyOffThreshold;
  int autoOnTime;
  int autoOffTime;
  int sensorDuration;
  float dividerRatio;         // Spannungsteiler-Verhältnis
  float zeroVoltage;          // Spannung bei 0 bar (nach Teiler)
  float spanVoltage;          // Spannung bei bekanntem Druck (nach Teiler)
  float knownPressure;        // Bekannter Druck für Span-Kalibrierung (bar)
  bool spanCalibrated;        // Span-Kalibrierung durchgeführt
};

Settings settings = {
  .pressureThreshold = 0.35f,
  .pressureScale = 1.0f,      // Wird bei Span-Kalibrierung berechnet
  .joyOnThreshold = 650,
  .joyOffThreshold = 420,
  .autoOnTime = 5000,
  .autoOffTime = 2000,
  .sensorDuration = 10000,
  .dividerRatio = 1.0f,        // 1.0 = kein Spannungsteiler
  .zeroVoltage = 0.0f,
  .spanVoltage = 0.0f,
  .knownPressure = 0.0f,
...
float pressUnits = 0.0f;         // Druck in bar (roh)
...
void calibratePressureZero(){
  ...
  settings.spanCalibrated = false; // Null-Kalibrierung invalidiert Span-Kalibrierung
  pressFilt = -1.0f;
  pressOK = false;
...
void calibratePressureSpan(float knownPressure){
  ...
  // Berechne Spannung nach Spannungsteiler
  float sensorVoltage = currentVoltage / settings.dividerRatio;
  
  // Berechne Spannung bei bekanntem Druck (nach Null-Abzug)
  float voltageAtPressure = sensorVoltage - settings.zeroVoltage;
  
  if(voltageAtPressure > 0.01f){ // Vermeide Division durch Null
    // Berechne Skalierungsfaktor: bar pro Volt
    settings.pressureScale = knownPressure / voltageAtPressure;
    ...
    // Fehler: Zu kleiner Spannungsunterschied
    setTxt(1, RED_COLOR);
...
void updatePressure(){
  ...
  // Berechne Sensorspannung (nach Spannungsteiler)
  float sensorVoltage = pressVolt / settings.dividerRatio;
  
  // Berechne Spannungsdifferenz zum Nullpunkt
  float dV = sensorVoltage - settings.zeroVoltage;
  
  // Berechne Druck in bar
  if(settings.spanCalibrated) {
    // Verwendung des berechneten Skalierungsfaktors
  ...
  // Keine negative Drücke anzeigen
...
void drawPressureGauge(){
  ...
  tft.printf("Schwelle: %.3f bar", settings.pressureThreshold);

  // Zeige Kalibrierungsstatus
  tft.setCursor(24, CONTENT_Y+128);
  if(settings.spanCalibrated){
    tft.printf("Scale: %.3f bar/V", settings.pressureScale);
    tft.setCursor(24, CONTENT_Y+146);
    tft.printf("Known: %.2f bar @ %.3fV", settings.knownPressure, settings.spanVoltage);
  } else {
    tft.print("Span: NICHT kalibriert!");
  }

  uint16_t sc = pressOK ? GREEN_COLOR : RED_COLOR;
  const char* st = pressOK ? "OK" : "NICHT OK";

  tft.fillRoundRect(18, CONTENT_Y+164, SCREEN_W-36, 44, 12, sc);
  tft.drawRoundRect(18, CONTENT_Y+164, SCREEN_W-36, 44, 12, ORANGE_LIGHT);
  setTxt(2, BLACK_COLOR);
  tft.setCursor(84, CONTENT_Y+178);
  tft.print(st);

  int barX = 24, barY = CONTENT_Y+212, barW = 192, barH = 14;
...
<body>
    <div class="container">
        ...
        <div class="grid">
            <div class="card">
                ...
                <div class="status-item">
                    <span class="status-label">Skala (bar/V):</span>
            ...
            <div class="card">
                ...
                <div class="setting-group">
                    <label class="setting-label">Druckschwelle (bar): <span id="thresholdValue">0.35</span></label>
                    <input type="range" id="threshold" min="0" max="5" step="0.01" value="0.35">
                </div>

                <div class="setting-group">
                    <label class="setting-label">Manuelle Druckskala (bar/V): <input type="number" id="scale" min="0.1" max="10" step="0.1" value="1.0"></label>
                </div>
                ...
                <div class="setting-group" style="margin-top: 20px;">
                    <h3>Span-Kalibrierung</h3>
                    <label class="setting-label">Bekannter Druck (bar): <input type="number" id="knownPressure" min="0.01" max="5.0" step="0.01" value="0.45"></label>
                    <button class="success" onclick="calibrateSpan()">Span-Kalibrierung durchführen</button>
                    <p style="font-size: 0.9em; color: #ffcc80; margin-top: 5px;">
                        Anleitung: 1. Null kalibrieren 2. Bekannten Druck anwenden 3. Span kalibrieren
                    </p>
                </div>
    ...
    <script>
        ...
        function updateStatus(data) {
            ...
            document.getElementById('pressure').textContent = data.pressureFilt.toFixed(3) + ' bar';
            ...
            document.getElementById('spanCal').textContent = data.spanCalibrated ? 'Ja' : 'Nein';
            document.getElementById('spanCal').className = 'status-value ' + (data.spanCalibrated ? 'status-ok' : 'status-notok');
        ...
        function calibrateSpan() {
            ...
            .then(data => { 
                if(data.status === 'calibrated') {
                    showNotification('Span-Kalibrierung erfolgreich!'); 
                } else {
                    showNotification('Fehler: ' + data.error, true);
                }
            })
            .catch(error => {
                showNotification('Fehler bei Span-Kalibrierung!', true);
                console.error('Error:', error);
            });
        }
...
void handleCalibrateSpan() {
  if (server.hasArg("plain")) {
    ...
    if (knownPressure > 0.0f && knownPressure <= 5.0f) {
      ...
      server.send(400, "application/json", "{\"error\":\"Invalid pressure value\"}");
    }
  } else {
    server.send(400, "application/json", "{\"error\":\"Invalid request\"}");
  }
