...
#define NVS_NAMESPACE "AmirMobasheraghdam uni bonn"
...
void drawHeader(const char* title){
  ...
  setTxt(2, TEXT_COLOR);
  tft.setCursor(10, 8);
  tft.print("AmirMobasheraghdam uni bonn");
...
void drawWaveform(){
  ...
  const int waveX = 125;      // NICHT 150 (zu nah an Relais-Anzeige)
  const int waveY = 10;
  const int waveW = 70;       // 125 + 70 = 195 < 200 (Relais startet bei 200)
  const int waveH = 24;
...
void calibratePressureZero(){
  ...
  pressFilt = -1.0f;  // Reset für Filter
  pressOK = false;
  
  // In NVS speichern (Preferences ist bereits geöffnet)
...
void updatePressure(){
  ...
  // Spannung zurückrechnen auf Sensor (mit Spannungsteiler)
  ...
  // Filter initialisieren wenn nötig
  if(pressFilt < 0) {
...
<body>
    ...
    <script>
        ...
        function updateStatus(data) {
            ...
            // Update wave data for oscilloscope
            ...
            // Update slider values if they haven't been touched
...
// ==================== NVS Initialisierung ====================
void loadSettings() {
  // Preferences ist bereits geöffnet, nur lesen
  ...
  settings.dividerRatio = preferences.getFloat("dividerRatio", 1.0f);  // Standard: kein Teiler
...
void updateWaveform() {
  if(millis() - lastWaveUpdate >= WAVE_UPDATE_MS){
    if(relayActive){
      // Sinus-ähnliche Welle für Motoraktivität
...
void setup(){
  ...
  // Einstellungen laden (Preferences ein Mal öffnen und offen lassen)
